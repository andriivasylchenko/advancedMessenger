...
CommonCache.prototype.set = function(key, value,ttl) {
    ...
    // lruSize is the cache size. Thus, for lruSize = 0, no set is done.
    if (key && ttl>=0 && this.lruSize > 0) {
...
CommonCache.prototype.del = function(key) {
    if (key && this.lruSize > 0) {
        this.cache.del(key);
        this.rebuildLRUList(key, true);
    }
}
...
CommonCache.prototype.getLRUList = function() {
    var str = '';
    var p = this.first;
    while (p) {
        str = p.value+","+str;

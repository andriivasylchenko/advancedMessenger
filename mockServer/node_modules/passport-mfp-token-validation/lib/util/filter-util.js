...
FilterUtil.getServerUrlFromVcap = function() {
    var mfpService = getmfpService();
    var serverUrl = mfpService && mfpService['credentials'] && mfpService['credentials']['serverUrl'];
    ...
    if (! mfpService) {
        ...
        for (var prop in vcapServices) {
            if (prop.indexOf('AdvancedMobileAccess') == 0 && vcapServices[prop].length>0) {
                mfpService = vcapServices[prop][0];
            }
        }
    }
